...
Transformer-based protein language models map an input sequence $x$ to an embedding $z = f_\theta(x) \in \mathbb{R}^d$. Given a database $\mathcal{D} = \{z_i\}_{i=1}^N$ and a query $z_q$, protein retrieval is formulated as nearest-neighbor search under a similarity score $s(z_q, z_i)$ such as cosine similarity or an inner product in the maximum inner product search (MIPS) setting. A naive top-$k$ search evaluates $s(z_q, z_i)$ for all $N$ items, with $O(Nd)$ time per query, and becomes prohibitive as $N$ grows.
...
\subsection{LLAST}
Unlike generic ANN methods such as LSH or product quantization, which operate on
...
We introdouced Latte Latent Alignment Search Tool(LLAST). We clustered protein sequences in a latent space using k means under cosine distance (via L2 row normalization), then applied agglomerative hierarchical clustering to obtain a tree index. This tree serves as a prefilter, pruning sequences with dissimilar latent vectors before downstream alignment. The number of clusters K was selected by Kneedle knee detection on the mean cosine k means cost curve, with adaptive 10 way interval refinement around the elbow. 
\begin{figure}
    \centering
...
We report a rerun of k{=}3 clustering to compare LATTE and ESM-2 embeddings. Silhouette scores (cosine) favor LATTE, and cross‑partition agreement metrics indicate strong consistency between the two representations\citep{Vieira2025Medium}.
...
Enforcing structural integrity via AlphaFold derived losses or contact maps is impractical at scale: AlphaFold requires 10 to 20 minutes per sequence, contact maps demand large memory and rely on limited known structures and require aligning for calculation. A more scalable approach is to leverage other protein language models (PLMs) such as ProtT5 or ProtBert, ideally their smaller variants like ESM or ESM-2, for structural supervision.


\section{Conclusion}\label{sec:conclusion}
LATTE’s GFP case study shows that a compact, structure-aware latent space's 256 dimensions with an active KL near 0.05 captures the functional and spectral determinants of fluorescent proteins without relying on sequence generation. A Gaussian process classifier trained on LATTE embeddings separates FPs from non-FPs with 0.987 5 fold cross validation accuracy, and a GP regressor predicts excitation and emission peaks with RMSEs of 2.70\,nm and 3.80\,nm, respectively. Manifold visualizations (t-SNE) reveal clear FP/non-FP separation and smooth spectral gradients across the latent manifold, consistent with embeddings that encode structural constraints.

Taken together, these results indicate that LATTE’s low dimensional latent representation is informative and structure aligned, enabling reliable property inference and providing a general path to extend property guided analysis to other functional families.
...
\textbf{CRediT taxonomy:} DA: Conceptualization, Methodology, Software, Writing-original draft, Project administration

\subsection*{Acknowledgements}
We thank Minjae Lee, Sihyeon Moon, and Jooyoung Jung for their assistance with manuscript proofreading and for organizing the GitHub data repository.
