...
BLAST, the NCBI sequence search system, is highly informative but its per query runtime scales roughly linearly with database size. In practice it still scans large portions of the library for each query, repeatedly revisiting near duplicate or highly similar sequences. 
Furthermore, the rapid advancement of AI and automation in biology has led to an exponential growth in sequence databases. According to NCBI RefSeq statistics, the number of protein sequences surged from approximately 130 million in 2019 to over 427 million by late 2025, a more than threefold increase driven largely by metagenomic and whole-genome shotgun data~\cite{ncbi_refseq_stats}. In this era of data explosion, linear-time search algorithms like BLAST face increasing scalability challenges.
...
To compare the global structure of the embedding spaces, we computed pairwise cosine distances (1 $-$ cosine similarity) on matched subsets and summarized both the distributions and their empirical cumulative distribution functions (ECDFs). LATTE latent distances are broader with a heavier tail (mean $=0.1694$, SD $=0.3428$, $p50 = 0.0141$, $p90 = 0.9006$; $n = 49{,}847$), whereas ESM-2 distances are tighter (mean $=0.0381$, SD $=0.0822$, $p50 = 0.00953$, $p90 = 0.1133$; $n = 49{,}847$) (Figure \ref{fig:pairwise-cosine}, Table~\ref{tab:pairwise-cosine-summary}). This suggests that ESM-2 clusters points more compactly, while LATTE yields a more diffuse geometry that can increase recall for remote neighbors; in practice, we combine LATTE’s structure-aware prefilter with sequence alignment (``LLAST'') to recover interpretability and precision.
...
\begin{table}[!ht]
  ...
  \begin{tabular}{crccc}
    \hline
    Top-$K$&$N_{\max}$ & Mean SeqRecall@50 & Mean candidates/query & Approx.\ reduction \\
    \hline
...
\caption{Sequence recall histograms of the prefiltering step for different Top-$K$ values (1--5). Each panel shows the distribution of per-query recall, i.e., the fraction of the baseline BLAST top hits that are preserved after latent-space prefiltering.}
...
heavier-tailed cosine distance distributions while preserving neighbor ordering and
...
latent-tree search is restricted to a fixed number of leaves (Top-$K$) and forwards at
...
Fourth, LLAST performs an approximate nearest-neighbor search through a discretized
tree index. The latent-tree traversal relies on centroid distances and hierarchical
pruning decisions, which introduce quantization effects: sequences that lie near
...
under a fixed budget, not to clone BLAST's full-database behavior. In this sense,
...
SeqRecall@50 while reducing the average BLAST workload by ×100$\sim$260 on a
$ 10^6$ sequence database, making exact replication of baseline BLAST hits a
poor trade-off compared with the achievable efficiency gains.

\subsection{Relation to MMseqs2 and sequence-based prefilters}

To clarify the position of LLAST, we compare it with \textbf{MMseqs2}, which is currently a state-of-the-art tool for large-scale sequence search and filtering~\cite{Steinegger2017MMseqs2,Steinegger2018Linclust}. LLAST is not intended to be a drop-in replacement for MMseqs2, but rather differs fundamentally in terms of \textbf{search basis} and \textbf{computational control}.

\textbf{1. Search Basis: Sequence vs.\ Latent Structure.} 
MMseqs2 employs a cascade of $k$-mer based filters and ungapped extensions. While highly optimized with SIMD and parallel execution, it essentially relies on \textbf{raw sequence string matching}. Consequently, it has limitations in detecting \emph{remote homologs} in the `Twilight Zone' (sequence identity $<$ 30\%), where sequence similarity is low but structural features are conserved. 
In contrast, LLAST operates in a \textbf{structure-aware latent space} learned by LATTE. The tree traversal is driven by cosine distances between latent vectors rather than string matching. This allows LLAST to effectively cluster and retrieve proteins that share structural or functional similarity even when their primary sequences have diverged significantly. Furthermore, the same latent representation is reused for reconstruction and property prediction, providing a shared latent geometry for multiple downstream tasks.

\textbf{2. Time Complexity and Scalability: $O(N)$ vs.\ $O(1)$.} 
A critical difference lies in scalability with respect to database size ($N$).
For MMseqs2, as $N$ increases, the number of chance $k$-mer matches (false positives) naturally grows, leading to a larger number of candidates to verify. Thus, its search time scales linearly with the database size ($O(N)$).
LLAST, however, enforces an \textbf{explicit candidate budget}. Regardless of how massive the database becomes, LLAST selectively visits only the top-$K$ leaf clusters in the latent tree. By limiting the candidate set forwarded to BLAST to a fixed size ($N_{\max}$), LLAST effectively bounds the search workload to near-constant time ($O(1)$) per query. This makes LLAST a more scalable solution for finding structure-based homologs in exponentially growing databases.

\textbf{3. Complementary Roles.} 
MMseqs2 is a standalone, high-throughput search engine integrating its own filtering and alignment. LLAST acts as a \textbf{budgeted re-ranking layer} that sits atop a learned index and delegates final scoring to BLAST. 
Therefore, while MMseqs2 is the method of choice for maximum throughput scanning based on sequence identity, LLAST offers a modern alternative for scenarios requiring structure-aware retrieval or strict computational budget control in ultra-large databases. A summary comparison is provided in Table~\ref{tab:comparison_tools}.

\begin{table}[htbp]
\caption{Comparison of BLAST, MMseqs2, and LLAST.}
\label{tab:comparison_tools}
\centering
\small
\begin{tabular}{lccc}
\toprule
\textbf{Feature} & \textbf{BLAST} & \textbf{MMseqs2} & \textbf{LLAST} \\
\midrule
\textbf{Time Complexity} & $O(N)$ & $O(N)$ & $\approx O(1)$ \\
\textbf{Basis} & Sequence & Sequence & Sequence + Structure \\
\textbf{Speed} & Slow & Fast & Fast \\
\textbf{Method} & Seed-and-Extend & $k$-mer Pre-filtering & Latent Space Clustering \\
\bottomrule
\end{tabular}
\end{table}
...
the ablation study on structural losses, these results indicate that LATTE’s low-dimensional latents are informative, structure-aligned representations suitable for
